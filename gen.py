import hmac, hashlib, requests, string, random, json, time, os, sys, base64, signal, threading, re, subprocess, importlib, logging
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("activation_log.txt")]
)

def generate_random_name(base="DIVAN"):
    designs = ['▲','ℳ','☆','°','ℛ','『','ツ','◇','༺','◆','웃','꧁','彡','★','ン','•','乂','⍤','유','ヅ','Ø','♪','Ƹ','⌂','シ','⊹','·','∞','♡','✦','✧','◈','▸','꧂','༻','࿐','ʜ','ɪ','ᴋ','ᴍ','ɴ','ꪆ','ꪀ','』','「','」','〖','〗','【','】','《','》','ッ','ジ','ヅ','亗','ℳ','ℛ','Ɽ','Ƈ','Ƨ','Ƴ','Ʀ','Ƶ','⋆','⋈']
...
class MultiRegionActivator:
    def __init__(self, max_workers=8, turbo_mode=True):
        ...
        self.regions = {
            ...
            'TW': {'guest_url': 'https://ffmconnect.live.gop.garenanow.com/oauth/guest/token/grant', 'major_login_url': 'https://loginbp.ggpolarbear.com/MajorLogin', 'get_login_data_url': 'https://clientbp.ggpolarbear.com/GetLoginData', 'client_host': 'clientbp.ggpolarbear.com'}
        }
    ...
    def clear_activation_log(self):
        try:
            with open("activation_log.txt", 'w') as f:
                f.write("")
        except: pass
    ...
    def generate_fingerprint(self):
        user_agents = ['Mozilla/5.0 (Linux; Android 10; SM-G973F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.114 Mobile Safari/537.36', 'Mozilla/5.0 (Linux; Android 11; Pixel 5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.115 Mobile Safari/537.36', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36']
        headers = {'User-Agent': random.choice(user_agents), 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8', 'Accept-Language': random.choice(['en-US,en;q=0.9', 'en-GB,en;q=0.8']), 'Accept-Encoding': 'gzip, deflate, br', 'Connection': 'keep-alive'}
        self.session.headers.update(headers)
        self.rotate_adapter()

    def smart_rate_limit_bypass(self):
        if self.turbo_mode: time.sleep(random.uniform(0.01, 0.05))
    ...
    def major_login(self, access_token, open_id, region='IND'):
        ...
        headers = {'X-Unity-Version': '2018.4.11f1', 'ReleaseVersion': 'OB53', 'Content-Type': 'application/x-www-form-urlencoded', 'X-GA': 'v1 1', 'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 7.1.2; ASUS_Z01QD Build/QKQ1.190825.002)', 'Host': host_header, 'Connection': 'Keep-Alive'}
        ...
        for attempt in range(max_attempts):
            ...
            if attempt < max_attempts - 1: self.advanced_retry_strategy(attempt, max_attempts)
            if region == 'VN' and attempt == 0:
                alt_url = "https://loginbp.ggblueshark.com/MajorLogin"
                alt_host = "loginbp.ggblueshark.com"
                headers['Host'] = alt_host
                url = alt_url
                print(f"     {Y_}[↺] VN region: retrying with {alt_host}{S_}")
        return None
    ...
    def GET_LOGIN_DATA(self, JWT_TOKEN, PAYLOAD, region='IND'):
        ...
        headers = {'Expect': '100-continue', 'Authorization': f'Bearer {JWT_TOKEN}', 'X-Unity-Version': '2018.4.11f1', 'X-GA': 'v1 1', 'ReleaseVersion': 'OB53', 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 9; G011A Build/PI)', 'Host': client_host, 'Connection': 'close', 'Accept-Encoding': 'gzip, deflate, br'}
        max_attempts = 2
        for attempt in range(max_attempts):
            try:
                self.smart_rate_limit_bypass()
                response = self.session.post(url, headers=headers, data=PAYLOAD, verify=False, timeout=8 if self.turbo_mode else 12)
                if response.status_code == 200:
                    return True
    ...
    def activate_single_account(self, account_data):
        ...
        region = self.selected_region if self.selected_region else account_data.get('region', 'IND')
        if region not in self.regions: region = 'IND'
...
def banner():
    ...
    print(divan_art)
    draw_line()
    print(f"{W_}{B_}      FF PREMIUM GENERATOR & ACTIVATOR (DIVAN'S EDITION){S_}")
    draw_line()
    print(f"{C_}👑 CREDITS: @DivanSingh & @FireXDecoder | Channel: @FireXDecoding{S_}")
    draw_line()
...
BASE_FOLDER = os.path.join(BASE_DIR, "DIVAN")
...
def safe_exit(signum=None, frame=None):
    global EXIT
    EXIT = True
    print(f"\n{G_}👋 Thank you for using DIVAN'S Account Generator! Goodbye!{S_}")
    sys.exit(0)

signal.signal(signal.SIGINT, safe_exit)
signal.signal(signal.SIGTERM, safe_exit)

def create_account(region, account_name, is_ghost=False):
    if EXIT: return None
    try:
        ...
        signature = hmac.new(api_key, payload_json.encode(), hashlib.sha256).hexdigest()
        ...
        response = requests.post(url, headers=headers, data=payload_json, timeout=30, verify=False)
        if response.status_code != 200: return None
        res_json = response.json()
        if res_json.get("code") != 0: return None
        uid = res_json["data"]["uid"]
        smart_delay()
        return get_token(uid, password, region, account_name, is_ghost)
    except: smart_delay(); return None

def get_token(uid, password, region, account_name, is_ghost=False):
    if EXIT: return None
    try:
        url = "https://100067.connect.garena.com/api/v2/oauth/guest/token:grant"
        payload = {"client_id": 100067, "client_secret": "2ee44819e9b4598845141067b281621874d0d5d7af9d8f7e00c1e54715b7d1e3", "client_type": 2, "password": password, "response_type": "token", "uid": uid}
        payload_json = json.dumps(payload, separators=(',', ':'))
        api_key = bytes.fromhex("2ee44819e9b4598845141067b281621874d0d5d7af9d8f7e00c1e54715b7d1e3")
        signature = hmac.new(api_key, payload_json.encode(), hashlib.sha256).hexdigest()
        headers = {"User-Agent": "GarenaMSDK/4.0.39(SM-A325M ;Android 13;en;HK;)", "Authorization": f"Signature {signature}", "Content-Type": "application/json; charset=utf-8", "Accept": "application/json", "Connection": "Keep-Alive", "Host": "100067.connect.garena.com"}
        response = requests.post(url, headers=headers, data=payload_json, timeout=30, verify=False)
        if response.status_code != 200: return None
        tok_json = response.json()
        if tok_json.get("code") != 0: return None
        open_id = tok_json["data"]["open_id"]
        access_token = tok_json["data"]["access_token"]
        encoded_field = encode_string(open_id)
        smart_delay()
        return major_register(access_token, open_id, encoded_field, uid, password, region, account_name, is_ghost)
    except: smart_delay(); return None

def major_register(access_token, open_id, field, uid, password, region, account_name, is_ghost=False):
    if EXIT: return None
    for attempt in range(10):
        try:
            ...
            headers = {"Accept-Encoding": "gzip", "Authorization": "Bearer", "Connection": "Keep-Alive", "Content-Type": "application/x-www-form-urlencoded", "Expect": "100-continue", "Host": "loginbp.ggpolarbear.com", "ReleaseVersion": "OB53", "User-Agent": "Dalvik/2.1.0 (Linux; U; Android 9; ASUS_I005DA Build/PI)", "X-GA": "v1 1", "X-Unity-Version": "2018.4.11f1"}
            requests.post(url, headers=headers, data=encrypted_payload, verify=False, timeout=30)
            login_result = major_login_new(uid, password, access_token, open_id, region, is_ghost)
                ...
                if second_dot != -1:
                    ...
                    try:
                        ...
                        if len(parts) >= 2:
                            ...
                            if account_id: return {"account_id": str(account_id), "jwt_token": jwt_token}
                    except: pass
        return {"account_id": "N/A", "jwt_token": ""}
    except: return {"account_id": "N/A", "jwt_token": ""}

def encrypt_api_new(plain_hex):
...
def force_region_bind(region, jwt_token):
    try:
        ...
        encrypted_data = encrypt_api_new(proto_data.hex())
        payload = bytes.fromhex(encrypted_data)
        headers = {'User-Agent': "Dalvik/2.1.0 (Linux; U; Android 12; M2101K7AG Build/SKQ1.210908.001)", 'Connection': "Keep-Alive", 'Accept-Encoding': "gzip", 'Content-Type': "application/x-www-form-urlencoded", 'Expect': "100-continue", 'Authorization': f"Bearer {jwt_token}", 'X-Unity-Version': "2018.4.11f1", 'X-GA': "v1 1", 'ReleaseVersion': "OB53"}
...
def save_normal_account(account_data, region, is_ghost=False):
    try:
        if is_ghost: filename = os.path.join(GHOST_ACCOUNTS, "ghost.json")
        else: filename = os.path.join(ACCOUNTS_FOLDER, f"accounts-{region}.json")
        entry = {'uid': account_data["uid"], 'password': account_data["password"], 'account_id': account_data.get("account_id", "N/A"), 'name': account_data["name"], 'region': "STAR" if is_ghost else region, 'date_created': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'thread_id': account_data.get('thread_id', 'N/A'), 'activation_status': account_data.get('activation_status', 'UNKNOWN')}
        with get_lock(filename) as lock:
            data = []
            if os.path.exists(filename):
                try:
            ...
            if account_data.get("account_id", "N/A") not in existing:
                data.append(entry)
                with open(filename + '.tmp', 'w', encoding='utf-8') as f: json.dump(data, f, indent=2, ensure_ascii=False)
                os.replace(filename + '.tmp', filename)
                return True
        return False
    except: return False
...
def save_jwt_token(account_data, jwt_token, region, is_ghost=False):
    try:
        if is_ghost: filename = os.path.join(GHOST_FOLDER, "tokens-ghost.json")
        else: filename = os.path.join(TOKENS_FOLDER, f"tokens-{region}.json")
        entry = {'uid': account_data["uid"], 'account_id': account_data.get("account_id", "N/A"), 'jwt_token': jwt_token, 'name': account_data["name"], 'password': account_data["password"], 'date_time': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'region': "STAR" if is_ghost else region, 'thread_id': account_data.get('thread_id', 'N/A')}
        with get_lock(filename) as lock:
            data = []
            if os.path.exists(filename):
                try:
...
def generate_single_account(region, account_name, total_accounts, thread_id, is_ghost=False):
    global OK, RARE_CNT, CPL_CNT, global_activator, NEXT_TO_PRINT, last_generation_time
    if EXIT: return None
    account_result = create_account(region, account_name, is_ghost)
    ...
    with PRINT_LOCK:
        ...
        if activation_status_msg: print(activation_status_msg)
        is_rare, rtype, reason, rscore = check_rarity(account_result)
        is_couple, creason, partner = check_couple(account_result, thread_id)
        if is_rare: print(f" {C_}[✨] {Y_}RARE DETECTED! {rtype} (Score: {rscore}){S_}")
        if is_couple and partner: print(f" {C_}[💑] {Y_}COUPLE DETECTED! Saved.{S_}")
        NEXT_TO_PRINT += 1
    if is_rare:
    ...
    if account_result.get('jwt_token'): save_jwt_token(account_result, account_result['jwt_token'], "GHOST" if is_ghost else region, is_ghost)
    return account_result

def worker_thread(region, account_name, total_accounts, thread_id, is_ghost=False):
    while not EXIT:
        with LOCK:
            if OK >= total_accounts: break
        res = generate_single_account(region, account_name, total_accounts, thread_id, is_ghost)
        if not res: time.sleep(0.1)
        else: time.sleep(0.01)

def monitor_generation(interval=30, idle_threshold=45):
    global last_generation_time, OK, total_target
    while not EXIT and OK < total_target:
        time.sleep(interval)
        if EXIT or OK >= total_target: break
        idle_duration = time.time() - last_generation_time
        if idle_duration >= idle_threshold:
            print(f"\n{Y_}⚠️  [MONITOR] No accounts generated for {idle_duration:.0f} seconds.{S_}")
            print(f"{C_}💡 SUGGESTION: Please turn on Aeroplane Mode, wait 10 seconds, then turn it off to refresh your network connection.{S_}")
            print(f"{C_}🔄 Generation continues in background...{S_}\n")
            last_generation_time = time.time()
...
def about_section():
    banner()
    print(f" {B_}{W_}📌 ABOUT DIVAN'S ACCOUNT GENERATOR{S_}")
    draw_line()
    print(f" {C_}[★] Version    : {W_}3.0 (Auto-Activation Edition){S_}")
    print(f" {C_}[★] Developers : {W_}@DivanSingh & @FireXDecoder{S_}")
    print(f" {C_}[★] Channel    : {W_}@FireXDecoding{S_}")
...
def generate_accounts_flow():
    global OK, RARE_CNT, CPL_CNT, global_activator, NEXT_TO_PRINT, EXIT, total_target, last_generation_time
    banner()
    ...
    name_prefix = input(f"{Y_} [>] {W_}Account Name Prefix (Default: DIVAN): {S_}").strip() or "DIVAN"
    while True:
        try:
    ...
    for i in range(thread_count):
        t = threading.Thread(target=worker_thread, args=(selected_region, name_prefix, account_count, i+1, is_ghost))
        t.daemon = True
        t.start()
        threads.append(t)
    monitor_thread = threading.Thread(target=monitor_generation, args=(30, 45), daemon=True)
